    ...
    <section class="content">
        <div class="container-fluid">
            <div class="card-header">
                <h5>Pilih File Restore (ekstensi sql)
                </h5>
            </div>
            <div class="card-body">
        ...
        if (move_uploaded_file($_FILES['file']['tmp_name'], $alamatfile)) {
            ...
            foreach ($lines as $line) {
                ...
                if (substr(trim($line), -1, 1) == ';') {
                    mysqli_report(MYSQLI_REPORT_STRICT); // Tambahkan satu baris
                    mysqli_query($koneksi, $templine) or print('<span class="badge badge-danger">Gagal memproses query :' . substr($templine, 0, 50) . '...' . '\': ' . $mysqli->error . '</span><br />');
